...
/*    protected final static Logger logger = new Logger(CourseComponent.class.getName());
    ...
    *//*@NonNull
    ...
    public boolean isMultiSections(){
        boolean IsMultiSection=false;
        if(children!=null && children.size()>0) {
            if (children.size() > 1)
                IsMultiSection = true;
        }
   ...
   /* //get all officemix
    public List<OfficeMixBlockModel> getOfficeMixs(){
        List<CourseComponent> officemixs = new ArrayList<>();
        fetchAllLeafComponents(officemixs, EnumSet.of(BlockType.OFFICEMIX));
        // Confirm that these are actually VideoBlockModel instances.
        // This is necessary because if for some reason the data is null,
        // then the block is represented as an HtmlBlockModel, even if
        // the type is video. This should not actually happen in practice
        // though; this is just a safeguard to handle that unlikely case.
        for (Iterator<CourseComponent> officemixIterator = officemixs.iterator();
             officemixIterator.hasNext();) {
            CourseComponent videoComponent = officemixIterator.next();
            if (!(videoComponent instanceof PDFBlockModel)) {
                officemixIterator.remove();
            }
        }
        return (List<OfficeMixBlockModel>)(List)officemixs;
    }*/
