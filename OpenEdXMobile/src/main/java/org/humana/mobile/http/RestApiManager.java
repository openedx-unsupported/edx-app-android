...
import org.humana.mobile.http.model.CourseIdObject;
...
public class RestApiManager implements IApi {
    ...
    public List<EnrolledCoursesResponse> getEnrolledCourses(boolean fetchFromCache) throws Exception {
        Random r = new Random();
        int i1 = (r.nextInt(99999999) + 10000000);
        if (!NetworkUtil.isConnected(context)) {
            return oauthRestApi.getEnrolledCourses(loginPrefs.getUsername(),String.valueOf(i1)).execute().body();
        } else if (fetchFromCache) {
            return oauthRestApi.getEnrolledCourses(loginPrefs.getUsername(),String.valueOf(i1)).execute().body();
        } else {
            return oauthRestApi.getEnrolledCoursesNoCache(loginPrefs.getUsername(),String.valueOf(i1)).execute().body();
        }
    }
