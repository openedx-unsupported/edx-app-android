...
class DbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE " + DbStructure.Table.DOWNLOADS
                ...
                + DbStructure.Column.BLOCK_ID + " TEXT, "
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 6) {
            upgradeDatabaseToV6(db, oldVersion, newVersion);
        }
    ...
    private void upgradeDatabaseToV6(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.beginTransaction();
        try {
