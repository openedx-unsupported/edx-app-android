...
import org.edx.mobile.inapppurchases.BillingProcessorKt;
...
public class CourseTabsDashboardFragment extends BaseFragment {
    ...
    private void setupIAPLayout() {
        ...
        boolean isPurchaseEnabled = environment.getFeaturesPrefs().isIAPEnabledForUser(environment.getLoginPrefs().isOddUserId());
    ...
    private void initIAPObservers() {
        iapViewModel.getProductPrice().observe(getViewLifecycleOwner(), new EventObserver<>(productDetails -> {
            setUpUpgradeButton(productDetails);
            return null;
        }));
    ...
    private void setUpUpgradeButton(ProductDetails.OneTimePurchaseOfferDetails productDetails) {
        ...
        binding.accessError.setPrimaryButtonText(ResourceUtil.getFormattedString(
                ...
                AppConstants.PRICE,
                productDetails.getFormattedPrice()
        ).toString());

        binding.accessError.setPrimaryButtonListener(view -> {
            iapAnalytics.trackIAPEvent(Analytics.Events.IAP_UPGRADE_NOW_CLICKED, "", "");
            iapViewModel.startPurchaseFlow(courseData.getCourseSku(), BillingProcessorKt.getPriceAmount(productDetails), productDetails.getPriceCurrencyCode());
        });
    }
    ...
    private View.OnClickListener onFindCourseClick() {
        return v -> {
            environment.getAnalyticsRegistry().trackUserFindsCourses(0);
    ...
    public void setupToolbar(boolean hasAccess) {
        ...
        if (hasAccess && courseData.isUpgradeable() && environment.getFeaturesPrefs().isValuePropEnabled()) {
            ...
            protected void onResponse(@NonNull final EnrolledCoursesResponse course) {
                if (getActivity() != null) {
                    ...
                    if (fullscreenLoader != null && fullscreenLoader.isResumed()) {
                        new SnackbarErrorNotification(binding.getRoot()).showUpgradeSuccessSnackbar(R.string.purchase_success_message);
