...
/**
 * This class will run all the tests defined in the ApiTests class on a mock
 * server
 */
public class MockApiTests extends ApiTests {
    ...
    protected void setUp() throws Exception {
        ...
        try {
            InputStream in = getInstrumentation().getTargetContext()
                    .getAssets().open("config/config.json");
        ...
        public MockResponse dispatch(RecordedRequest recordedRequest)
            ...
            if ("POST".equals(method)) {
                if (urlMatches(path, "/oauth2/access_token")) {
                    ...
                    try {
                        JSONObject body = new JSONObject(recordedRequest.getBody().readUtf8());
